...
class GameScene extends JSONLevelScene {
  ...
  nextTurn() {
    ...
    if (this.current_unit.active) {
      ...
      if (this.current_unit.type === "player_unit") {
        this.prefabs.show_player_unit.highlight_turn_unit(
          this.current_unit.party_key
        );
      }
  ...
  createNewEnemy() {
    ...
    for (let enemy_unit_name in encounter.enemy_data) {
      ...
      console.log("Enemy created: ", this.prefabs[enemy_unit_name]);
  ...
  rewards() {
    ...
    this.groups.player_units.children.each(function (player_unit) {
      player_unit.receive_experience(
        received_experience / this.groups.player_units.children.size
      );
      ...
      this.player_data.party_data[_key].current_level =
        player_unit.current_level;
    }, this);
